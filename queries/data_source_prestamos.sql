...
case when p.estatus_prestamo = 0 and p.estatus_ampliacion = 0 and a.Mod_calculo_interes = 1 and p.sesiones_restantes > 0 then a.Tasa_interes
     when p.estatus_prestamo = 0 and p.estatus_ampliacion = 0 and a.Mod_calculo_interes = 1 and p.sesiones_restantes <= 0 then a.Tasa_interes + a.interes_morosidad
     when p.estatus_prestamo = 0 and p.estatus_ampliacion = 1 and a.Mod_calculo_interes = 1 and p.sesiones_restantes > 0 then a.Tasa_interes + a.interes_ampliacion
     when p.estatus_prestamo = 0 and p.estatus_ampliacion = 1 and a.Mod_calculo_interes = 1 and p.sesiones_restantes <= 0 then a.Tasa_interes+ + a.interes_ampliacion+a.interes_morosidad
     else 0 end 
