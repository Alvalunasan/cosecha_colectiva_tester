...
sum(socio_grupo.acciones) as acciones,
case when sum(socio_grupo.acciones) > 0 then socio_prestamos.sum_monto_prestamo / sum(socio_grupo.acciones) else NULL end as prestamo_vs_ahorro,
socio_multas.*,
socio_asistencias.*,
socio_ganancias.*,
socio_transacciones.*
